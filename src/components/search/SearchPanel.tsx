...
class SearchPanel extends React.PureComponent<SearchPanelProps> {
  ...
  handleSearchDatasets = () => {
    ...
    if (
      ...
      this.props.datasetQuery.searchExpr != ""
    ) {
      productNames.forEach((product) => {
        if (this.props.datasetQuery.searchExpr == product.name.toLowerCase()) {
          this.message =
            "For " +
            this.props.datasetQuery.searchExpr +
            ", please use the product search combo box on the right.";
          this.props.datasetQuery.searchExpr = null;
  ...
  makeProductSuggestions = () => {
    if (this.props.datasetQuery.productGroupNames.length == 0) {
      return this.props.serverInfo["products"].map((item: Product) => {
        return item.name;
      });
    }
  ...
  makeFieldSuggestions = () => {
    return this.metaInfo.fields.map((item) => {
      return item;
    });
  };
  ...
  getProductGroups = () => {
    return this.props.serverInfo["productGroups"].map((pg: ProductGroup) => {
      return pg.name;
    });
  };

  getSelectedProducts = () => {
    /* todo se 2024.03.08 --> rename? Der Kontext ist doch Produkt Gruppen und nicht Produkte */
      ...
      <Grid container spacing={2}>
          ...
          <Stack spacing={0.8} direction={"row"}>
            <DatePicker
              ...
              renderInput={(params) => (
                <TextField
                  size={"small"}
                  sx={{ minWidth: "175px", maxWidth: "175px" }}
                  {...params}
                  helperText={null}
                />
              )}
            ...
            <MultipleSelectTextField
              ...
              width="300px"
              size={"small"}
              selectedItems={this.props.datasetQuery.metadatafields}
            ></MultipleSelectTextField>
            <TextField
              id={"lucene-search"}
              key={"lucene-search"}
        ...
        <Grid item xs={8} md={12}>
          <Stack direction={"row"} alignItems="center">
            ...
            <IconButton onClick={this.props.openHelpDialog}>
